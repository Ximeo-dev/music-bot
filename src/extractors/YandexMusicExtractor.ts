...
export class YandexMusicExtractor extends BaseExtractor {
	...
	private YaRegex: YaRegex = {
		...
		playlist:
			/(^https:)\/\/music\.yandex\.[A-Za-z]+\/users\/[A-Za-z0-9]+\/playlists\/[0-9]+/,
		album: /(^https:)\/\/music\.yandex\.[A-Za-z]+\/album\/[0-9]+/,
	}

	private buildTrack = (track: Types.Track, context: any | null) =>
		new Track(this.context.player, {
			title: track.title,
			raw: track,
			description: `Genre: ${track.albums[0].genre}, Release year: ${
				track.albums[0].year
			}, Explicit: ${
			...
			source: 'arbitrary',
			thumbnail: `https://${track.coverUri.replace('%%', '600x600')}`,
	...
	async validate(query: string): Promise<boolean> {
		...
		return (
			this.YaRegex.track.test(query) ||
			this.YaRegex.playlist.test(query) ||
			this.YaRegex.album.test(query)
		)
	}

	async handle(query: string, context: any): Promise<ExtractorInfo> {
		if (this.YaRegex.track.test(query)) {
			context.type = 'ym_track'
		} else if (this.YaRegex.album.test(query)) {
			context.type = 'ym_album'
		} else if (this.YaRegex.playlist.test(query)) {
			context.type = 'ym_playlist'
		} else context.type = 'ym_search'

		if (context.type === 'ym_album') {
			...
			const playlist = new Playlist(this.context.player, {
				...
				thumbnail: `https://${albumonly.coverUri.replace('%%', '600x600')}`,
		...
		if (context.type === 'ym_search') {
			const data = (await this.YM.searchTracks(query, { pageSize: 10 })).tracks
				.results
			const tracks = data
				?.filter(track => track.available)
				.map(track => {
	...
	async getRelatedTracks(track: Track): Promise<ExtractorInfo> {
		...
		if (simmilar.simmilarTracks.length === 0) {
			simmilarTracks = (
				await this.YM.getStationTracks(
					`genre:${simmilar.track.albums[0].genre}`
				)
			).sequence.map(sttrack => sttrack.track)
		}
		const playlist = new Playlist(this.context.player, {
	...
	public async getRadioTracks(
		stationId: string,
		queue?: string
	): Promise<ExtractorInfo> {
		const radio = await this.YM.getStationTracks(stationId, queue)
		const tracks = radio.sequence.map(track =>
			this.buildTrack(track.track, null)
		)
		const playlist = new Playlist(this.context.player, {
